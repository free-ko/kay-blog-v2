...
description: State를 잘 다루는 방법을 생각해보자.
published: true
---
...
- 여러 연관된 state를 만들어서 관리하는게 아니라, 하나의 불변의 값으로 관리
  ...
  const FlatState = () => {
  	...
  	const fetchData = () => {
  		...
  		.then(() => {
  			// fetch Data 성공
  			setPromiseState(PROMISE_STATE.FNISH);
  		})
  		.catch(() => {
  			// fetch Data 실패
  	...
  	if (promiseState === PROMISE_STATE.FNISH) return <FinishComponent />

...
1. `DOM + CSSOM`: 웹 브라우저 화면을 그리는데 필요한 리소스(HTML, CSS)를 다운로드 하여 브라우저가 이해할 수 있는 형태(관계 트리)로 변환하는 과정
2. `Layout`: 화면 구성 요소의 위차나 크기를 계산하고, 해당 위치에 요소를 배치하는 과정
3. `Paint`: 화면에 배치된 요소에 색을 채워 넣는 과정(이때, 레이어를 분리해서 작업)
4. `Composite`: 페인트 단계에서 쪼개 놓는 레이어를 합치는 과정
